        ...
        it('should test JSON parsing with malformed data', () => {
            cy.window().then((win) => {
                if (typeof win.parseJson === 'function') {
                    ...
                    // parseJson returns empty object for invalid JSON
                    expect(win.parseJson('{invalid json}')).to.be.an('object');
                    expect(win.parseJson('{"unclosed": "object"')).to.be.an('object');
                    expect(win.parseJson('')).to.be.an('object');
